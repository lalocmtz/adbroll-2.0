  ...
  return <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="space-y-4">
        ...
        <Card className="p-4 space-y-4">
          ...
          <Button onClick={handleAdaptScript} disabled={!selectedBrandId || isAdapting} className="w-full gap-2">
            <Sparkles className="w-4 h-4" />
            {isAdapting ? "Adaptando..." : "Adaptar por IA"}
          </Button>
        </Card>

        {showSaveTemplate ? <Card className="p-4 space-y-3">
            <Label>Nombre de la plantilla</Label>
            <Input value={templateName} onChange={e => setTemplateName(e.target.value)} placeholder="Ej: Hook + Problema + CTA" />
            <div className="flex gap-2">
              <Button onClick={handleSaveTemplate} size="sm" className="flex-1">
                Guardar
              </Button>
              <Button onClick={() => setShowSaveTemplate(false)} variant="outline" size="sm">
                Cancelar
              </Button>
            </div>
          </Card> : <Button variant="outline" onClick={() => setShowSaveTemplate(true)} className="w-full gap-2">
      ...
      <div className="lg:col-span-2 space-y-6">
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {sections.map((section, index) => <Badge key={index} variant="outline">
                {section.type}
              </Badge>)}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div className="space-y-3">
              {sections.map((section, index) => <Card key={index} className="p-3">
                  <div className="flex items-center gap-2 mb-2">
                    <Badge variant="secondary" className="text-xs">
                      {section.type}
                    </Badge>
                  </div>
                  <Textarea value={section.text} onChange={e => handleTextChange(index, 'text', e.target.value)} className="min-h-[80px] resize-none" />
                </Card>)}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {sections.map((section, index) => <Card key={index} className="p-3">
                  ...
                  <Textarea value={section.adaptedText || section.text} onChange={e => handleTextChange(index, 'adaptedText', e.target.value)} className="min-h-[80px] resize-none" placeholder="Adaptación por IA aparecerá aquí" />
                </Card>)}
            </div>
          </div>
        </div>
