...
import { Link2, Loader2, FileText, Mic, Film, Sparkles, Package, FolderOpen, Menu } from "lucide-react";
...
const urlSchema = z.string().url("Ingresa una URL válida").refine(url => {
  return url.includes("tiktok.com") || url.includes("instagram.com") || url.includes("youtube.com") || url.includes("youtu.be") || url.includes("facebook.com") || url.includes("fb.watch") || url.includes("threads.net") || url.includes("vimeo.com") || url.includes("reddit.com");
}, {
  message: "Solo enlaces de redes sociales (TikTok, Instagram, YouTube, Facebook, Threads, Vimeo, Reddit)"
});
type WorkflowState = "IDLE" | "ANALYZING_LINK" | "SCRIPT_READY" | "VOICE_READY" | "CLIPS_READY" | "VARIANTS_READY";
...
export function DashboardStudio() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("brands").select("id, name, product_description, tone_of_voice").order("name");
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from("video_analyses").select("*").eq("id", analysisId).single();
      if (error) throw error;
      return data;
    },
    enabled: !!analysisId,
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const {
        data: extractData,
        error: extractError
      } = await supabase.functions.invoke("extract-audio-from-link", {
        body: {
          video_url: videoUrl,
          brand_id: null
        }
  ...
  const canAccessTab = (tab: string): boolean => {
    switch (tab) {
      ...
      case "voz":
        return workflowState === "SCRIPT_READY" || workflowState === "VOICE_READY" || workflowState === "CLIPS_READY" || workflowState === "VARIANTS_READY";
      case "brolls":
        return workflowState === "VOICE_READY" || workflowState === "CLIPS_READY" || workflowState === "VARIANTS_READY";
      case "variantes":
    ...
    return <Card className="p-8">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <div className="flex gap-3 mb-4">
            <Input type="url" placeholder="https://www.tiktok.com/@... o cualquier red social" value={videoUrl} onChange={e => setVideoUrl(e.target.value)} disabled={loading} className="flex-1" onKeyDown={e => e.key === "Enter" && handleAnalyze()} />
  ...
  return <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          {sections.length > 0 && <p className="text-muted-foreground">
          ...
          <SheetContent side="left" className="w-80">
            <div className="space-y-6 mt-6">
              <div>
                ...
                <div className="space-y-2">
                  {brands?.map(brand => <Button key={brand.id} variant={selectedBrandId === brand.id ? "default" : "outline"} className="w-full justify-start" onClick={() => setSelectedBrandId(brand.id)}>
                      {brand.name}
                    </Button>)}
                </div>
              </div>
      ...
      <Card className="p-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-4">
            <TabsTrigger value="guion" disabled={!canAccessTab("guion")} className="gap-2">
              <FileText className="w-4 h-4" />
              Guion
            </TabsTrigger>
            <TabsTrigger value="voz" disabled={!canAccessTab("voz")} className="gap-2">
              <Mic className="w-4 h-4" />
              Voz
            </TabsTrigger>
            <TabsTrigger value="brolls" disabled={!canAccessTab("brolls")} className="gap-2">
              <Film className="w-4 h-4" />
              B-rolls y subtítulos
            </TabsTrigger>
            <TabsTrigger value="variantes" disabled={!canAccessTab("variantes")} className="gap-2">
          ...
          <TabsContent value="guion" className="mt-6">
            <ScriptTab sections={sections} onSectionsChange={setSections} videoThumbnail={(analysis?.metadata as any)?.thumbnail_url} videoDuration={(analysis?.metadata as any)?.duration} detectedLanguage="es-MX" selectedBrandId={selectedBrandId} brands={brands || []} onBrandChange={setSelectedBrandId} onNext={() => {
            setActiveTab("voz");
            setWorkflowState("SCRIPT_READY");
          ...
          <TabsContent value="voz" className="mt-6">
            <VoiceTab sections={sections} selectedVoiceId={selectedVoiceId} onVoiceChange={setSelectedVoiceId} voiceSettings={voiceSettings} onVoiceSettingsChange={setVoiceSettings} voiceoverUrl={voiceoverUrl} onVoiceoverGenerated={setVoiceoverUrl} onNext={() => {
            setActiveTab("brolls");
          ...
          <TabsContent value="brolls" className="mt-6">
            <ClipsTab sections={sections} brandId={selectedBrandId} assignments={clipAssignments} onAssignmentsChange={setClipAssignments} hookText={hookText} onHookTextChange={setHookText} subtitleStyle={subtitleStyle} onSubtitleStyleChange={setSubtitleStyle} onNext={() => {
          ...
          <TabsContent value="variantes" className="mt-6">
            <VariantsTab variantCount={variantCount} onVariantCountChange={setVariantCount} variantOptions={variantOptions} onVariantOptionsChange={setVariantOptions} onRender={async () => {
